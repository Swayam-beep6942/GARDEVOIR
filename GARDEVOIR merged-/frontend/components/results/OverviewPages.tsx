...
export function ScorePage({
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      {meta.warning && (
        <div className="lg lg-static mb-5 px-6 py-5 text-center" style={{ borderRadius: 20, borderColor: "rgba(255,80,80,0.35)" }}>
          ...
          <p className="mt-2 text-sm text-white/55">This site is not adequately protected. Start with the vulnerable regions.</p>
        </div>
      )}

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
        <div className="lg lg-static lg-frost p-8 text-center lg:col-span-5" style={{ borderRadius: 24 }}>
          ...
          {scoring.total_points_lost > 0 && (
            <button
              onClick={onWhyPoints}
              className="lg lg-frost mt-6 w-full py-3 text-white font-semibold uppercase"
        ...
        <div className="lg lg-static lg-frost p-6 lg:col-span-7" style={{ borderRadius: 24 }}>
...
export function RegionsPage({ scan }: { scan: ScanReport }) {
  const scoring = scan.scoring;
  if (!scoring) return null;
  return (
    <div className="mx-auto w-full max-w-5xl">
...
export function AnalystPage({ scan }: { scan: ScanReport }) {
  const aiSummary = scan.ai_summary;
  if (!aiSummary) {
    return (
      <div className="lg lg-static mx-auto max-w-lg p-8 text-center" style={{ borderRadius: 24 }}>
        <p className="uppercase text-white/40" style={{ letterSpacing: "0.22em", fontSize: "0.62rem" }}>Analyst</p>
        <p className="mt-3 text-white/60">No analyst summary for this assessment.</p>
      </div>
    );
  }
